...
async function iniciarChat() {
  ...
  console.log("[IA] Cargando métricas del proyecto desde la base de datos...");
  let metricas;
  try {
    metricas = await obtenerMetricas();
  } catch (err) {
    console.error("[Error] Error al conectar con la base de datos:", err.message);
    process.exit(1);
  }
  console.log(`[OK] ${metricas.length} tareas cargadas. ¡Listo para conversar!\n`);
  ...
  const preguntar = () => {
    rl.question("Tú: ", async (input) => {
      ...
      try {
        ...
        console.error("[Error] Error al conectar con la IA:", error.message, "\n");
      }
