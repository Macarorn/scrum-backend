...
const solicitudService = {
  ...
  async crearSolicitud({ id_usuario, id_proyecto, mensaje_opcional }) {
    ...
    if (!(await proyectoActivo(id_proyecto))) {
      return { status: 400, data: null, message: 'Proyecto inactivo o eliminado' };
    }
    ...
    if (solCount[0].total >= 5) {
      return { status: 429, data: null, message: 'Límite de 5 solicitudes cada 6 horas alcanzado' };
    }
    ...
    return { status: 201, data: { id: result.insertId }, message: 'Solicitud creada' };
  },
  ...
  async listarTodas({ id_usuario }) {
    const [rows] = await pool.query(
      `SELECT * FROM solicitud WHERE id_usuario = ?`,
      [id_usuario]
    );
  ...
  async aprobarSolicitud({ id_usuario_aprobador, id_solicitud, id_rol }) {
    ...
    if (solicitud.estado !== 'Pendiente') {
      return { status: 409, data: null, message: 'Solo puedes aprobar solicitudes pendientes' };
    }
  ...
  async rechazarSolicitud({ id_usuario_aprobador, id_solicitud, motivo }) {
    ...
    if (solicitud.estado !== 'Pendiente') {
      return { status: 409, data: null, message: 'Solo puedes rechazar solicitudes pendientes' };
    }
  ...
  async cancelarSolicitud({ id_usuario, id_solicitud }) {
    ...
    if (solicitud.id_usuario !== id_usuario) {
      return { status: 403, data: null, message: 'No puedes cancelar esta solicitud' };
    }

    if (solicitud.estado !== 'Pendiente') {
      return { status: 409, data: null, message: 'Solo puedes cancelar solicitudes pendientes' };
    }
  ...
  // INVITAR
  // =============================
  async invitarUsuario({ id_usuario_aprobador, id_usuario, id_proyecto, id_rol }) {
