...
// Lista tareas filtrando por sprint o estado si vienen en query params.
export const listarTareas = async (req, res, next) => {
    try {
        const { sprintId, estado } = req.query;
        const data = listarTareasPorSprint(sprintId, estado);
...
export const actualizarTarea = async (req, res, next) => {
    try {
        ...
        const data = actualizarTareaPorId(req.params.id, req.body, userIdFromReq(req));
        res.status(200).json({
            success: true,
            data,
