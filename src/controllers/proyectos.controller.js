...
export const actualizarProyecto = async (req, res, next) => {
  try {
    const errores = validarProyecto(req.body);
    if (errores.length) return res.status(400).json({ success: false, error: "validation_error", message: "Datos inválidos", details: errores });
    const data = await proyectosService.actualizarProyecto(req.params.id, req.body);
    res.status(200).json({ success: true, data, message: "Proyecto actualizado" });
  } catch (error) {
    next(error);
  }
};
...
export const eliminarProyecto = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ success: true, data, message: "Proyecto eliminado" });
  } catch (error) {
    next(error);
  }
};